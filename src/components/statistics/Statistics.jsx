...
const Statistics = () => {
    ...
    useEffect(() => {
        if (!loading) {
            ...
            const stats = lessons.map(lesson => {
                ...
                if (status.quizScore > highScore) highScore = status.quizScore;
    ...
    return (
        <div
            className="w-full max-w-4xl mx-auto bg-white self-start rounded-lg overflow-hidden"
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-neutral-700">
                <StatCard
                    ...
                    icon={<BarChart3 className="text-blue-500 dark:text-blue-300" size={24} />}
                />
                <StatCard
                    title="الدروس المكتملة"
                    value={`${completedLessons} / ${lessons.length}`}
                    icon={<Check className="text-blue-500 dark:text-blue-300" size={24} />}
                />
                <StatCard
                    title="أعلى درجة أختبار"
                    value={`${highestScore}%`}
                    icon={<Trophy className="text-blue-500 dark:text-blue-300" size={24} />}
                />
            </div>
